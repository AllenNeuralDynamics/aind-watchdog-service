...
class TestRunSubprocess(unittest.TestCase):
    ...
    def test_run_subprocess(self, mock_subproc: MagicMock):
        ...
        mock_subproc.return_value=subprocess.CompletedProcess(args=cmd, returncode=0, stdout=b"Mock stdout", stderr=b"Mock stderr")

        # Call the function
        result = run_job.run_subprocess(cmd)

        # Assert that mock_subproc was called with the correct arguments
        mock_subproc.assert_called_once_with(cmd, check=False, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
    ...
    def test_windows_call(self):
        ...
        with patch.object(Path, "exists") as mock_file:
            ...
            run_job.run_subprocess = MagicMock(return_value=subprocess.CompletedProcess(args=[],returncode=0))
        ...
        with patch.object(Path, "exists") as mock_file:
            mock_file.return_value = True
            run_job.run_subprocess = MagicMock(return_value=subprocess.CompletedProcess(args=[],returncode=1))
            winx_dir = run_job.execute_windows_command(src_dir, dest)
            winx_file = run_job.execute_windows_command(src_file, dest)
            self.assertEqual(winx_dir, False)
            self.assertEqual(winx_file, False)
...
class TestCopyToVast(unittest.TestCase):
    ...
    def test_copy_to_vast(self, mock_join: MagicMock, mock_mkdir: MagicMock, mock_execute_windows: MagicMock, mock_execute_linux: MagicMock, mock_alert: MagicMock):
